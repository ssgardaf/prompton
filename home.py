...
with col1:
    with st.expander("기본 정보", expanded=True):
        ...
        bmi = st.number_input("체질량지수 (BMI)", min_value=0.0, max_value=100.0, value=weight / ((height / 100) ** 2) if height > 0 else 0.0, step=0.1, help="체질량지수를 입력하세요.")
with col2:
    with st.expander("혈압 및 심박수", expanded=True):
...
with col3:
    with st.expander("혈당 및 콜레스테롤", expanded=True):
...
with col4:
    with st.expander("간 및 기타 수치", expanded=True):
